...
def init(self, creator: str = "Unknown"):
...
super().init(placeholder="Select a Mutation Forge service...", options=options, custom_id="v:mutation_forge_select")
...
def init(self, t1: int, t2: int, mm: int):
super().init(timeout=None)
self.t1 = t1
self.t2 = t2
self.mm = mm
self.confirmed: set = set()
b1 = discord.ui.Button(label="✅ Confirm Trade (Trader 1)", style=discord.ButtonStyle.success, custom_id=f"trade_t1_{t1}{t2}")
b2 = discord.ui.Button(label="✅ Confirm Trade (Trader 2)", style=discord.ButtonStyle.success, custom_id=f"trade_t2{t1}_{t2}")
...
def init(self, target, author, message=None):
super().init(timeout=60.0)
self.target = target
self.author = author
